<template>
  ...
  <Modal :visible="isModalVisible" @close="closeModal">
    <h1 style="color: black">Datos Sísmicos</h1>
    <SeismicChart v-if="chartData && chartData.datasets && chartData.datasets.length" :chartData="chartData" />
  </Modal>
  <Modal :visible="isLoadingModalVisible" @close="closeLoadingModal">
    <h1>Please note:</h1>
    <p>
      This project contains complex graphics and heavy resources, which may take
      a while to load correctly. We appreciate your patience while everything
      gets set up for the best experience.
    </p>
  </Modal>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      // Add an array to store all transmission lines
  ...
  mounted() {
    // Initialize Three.js objects as component properties
    this.scene = null;
    this.camera = null;
    this.renderer = null;
    this.controls = null;
    ...
    fetch("http://{IP}:{PORT}/v1/api/example/generate_data")
    ...
    socket.on("data", (data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        this.satellitesData[i].push(...data[i]);
      }
      //console.log("Satellite data updated:", this.satellitesData);
  ...
  methods: {
    ...
    updateChartData(data) {
      ...
      //Log first 10 elements of times
      //console.log("First 10 elements of times:", times.slice(0, 10));
      const velocities = data.map(tuple => tuple[1]);
      //Log first 10 elements of velocities
      //console.log("First 10 elements of velocities:", velocities.slice(0, 10));

      let prediction = this.predictedArray[this.satelliteSelected];
      let verticalLines = [];

      let verticalLineX = 600;
      ...
      if (prediction) {
        ...
        verticalLines = [prediction.start, prediction.end];
          let annotations = {
          line1: {
            type: "line",
            xMin: verticalLines[0],
            xMax: verticalLines[0],
            borderColor: "red",
            borderWidth: 6,
          },
          line2: {
            type: "line",
            xMin: verticalLines[1],
            xMax: verticalLines[1],
            borderColor: "blue",
            borderWidth: 6,
          },
        };
    ...
    getColor(index) {
      // Puedes definir una paleta de colores fija o generar colores dinámicamente
